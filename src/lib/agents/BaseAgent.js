...
export class BaseAgent {
  ...
  async respond(contents, modulations = [], reflectionSummary = '') {
    // Apply modulations and inject reflection summary into system prompt
    const modulatedPrompt = this.applyModulations(this.systemPrompt, modulations, reflectionSummary);
  ...
  applyModulations(basePrompt, modulations, reflectionSummary = '') {
    let modulatedPrompt = basePrompt;

    // 1. Inject User Reflection Summary (Context)
    // Only inject if it exists (Orchestrator decided it was needed)
    if (reflectionSummary) {
      modulatedPrompt += '\n\n--- USER REFLECTION PROFILE ---\n';
      modulatedPrompt += 'The following is a summary of the user\'s past reflections, patterns, and insights. Use this to provide personalized support, but do not explicitly mention "your profile says" unless relevant:\n\n';
      modulatedPrompt += reflectionSummary;
      modulatedPrompt += '\n';
    }

    // 2. Apply Behavioral Modulations
